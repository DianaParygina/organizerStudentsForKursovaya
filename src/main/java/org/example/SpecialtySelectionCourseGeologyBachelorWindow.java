...
public class SpecialtySelectionCourseGeologyBachelorWindow extends JFrame {

    private final JTable specialtyGeologyTable;
    private final DefaultTableModel specialtyGeologyTableModel;

    public SpecialtySelectionCourseGeologyBachelorWindow(String title) {
            ...
            public void mouseClicked(MouseEvent e) {
                if (e.getClickCount() == 2) { // getClickCount() -  верный метод для определения двойного клика
                    ...
                    if (selectedRow != -1) {
                        ...
                        if (specialtyId == 1) {
                            new SpecialtySelectionCourseGeologySpecialtyWindow("Специалитет").setVisible(true);
                        } else if (specialtyId == 2) {
                            new SpecialtySelectionCourseGeologyBachelorWindow("Бакалавриат").setVisible(true);
                        }
                    }
                }
            }
